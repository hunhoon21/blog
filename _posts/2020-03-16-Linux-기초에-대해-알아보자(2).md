__Linux 기초에 대해 알아보자__ 시리즈 2번째 편입니다. 개인적으로 시리즈를 준비하며 사용법만 알았던 부분의 의미까지 이해할 수 있어 좋은 경험이었습니다. 
...
* find *.log
    * 성능 줄어듬 디렉토리를 직접 뒤짐
* find —help | head
* find / -name *.log 이름으로 찾겠다 접근권한 없는 디렉토리는 못 봄
* find ~ -type f -name *.log f는 파일 디렉토리는 d
* find . -type f -name “tecmint.txt” -exec rm -f {} \; 해당 파일들 다 삭제하는 경우

* Whereis 실행파일을 찾아줌
    * whereis ls, 위치와 매뉴얼 위치
    * 근데 ls 어떻게 바로 실행돼? 원래 현재경로의 파일만 실행 가능하잖아.
    * echo $PATH 환경변수
        * 경로들이 나옴. 환경변수 path들임
